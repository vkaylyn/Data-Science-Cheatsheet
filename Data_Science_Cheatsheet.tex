...
\textbf{Binomial} - number of successes $x$ in $n$ independent events, each with $p$ probability $\to \binom{n}x p^x q^{n-x}$, with $\mu = np$ and $\delta^2 = np(1-p)$. If n = 1, this is Bernoulli.
...
\end{itemize}
>>>>>>> Stashed changes
\textbf{Hypergeometric} - $x$ successes in a sample of size $n$, no replacement, from a size $N$ population with $X$ items of that feature
\vspace{.5mm}

\textbf{Geometric} - first success with $p$ probability on the $n^{th}$ trial $\to q^{n-1}p$, with $\mu = 1/p$ and $\sigma^2 = \frac{1-p}{p^2}$

\textbf{Negative Binomial} - X is the number of independent trials before $r$ successes
